...
public class IronMaceItem extends MaceItem {
    private static final float ATTACK_DAMAGE_MODIFIER_VALUE = 11.5F; // 自定义攻击伤害
    ...
    private static final float KNOCKBACK_POWER = 0.392F; // 自定义击退力量

    public IronMaceItem(Item.Settings settings) {
        super(settings);
    }
    ...
    private float calculateBonusDamage(float fallDistance) {
        if (fallDistance <= 3.0F) {
            return 11.5F * fallDistance;
        } else if (fallDistance <= 8.0F) {
            return 11.5F + 1.2F + 3.4F * (fallDistance * 3.5F);
        } else {
            return 11.5F + fallDistance * 1.87F;
        }
    }
    ...
    private static void knockbackNearbyEntities(World world, PlayerEntity player, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(8.0),
            ...
            entity.addVelocity(knockbackVec.x, 0.675464644, knockbackVec.z);
    ...
    private static Predicate<LivingEntity> getKnockbackPredicate(PlayerEntity player, Entity attacked) {
        return entity -> !entity.isSpectator() && entity != player && entity != attacked && !player.isTeammate(entity) &&
                attacked.squaredDistanceTo(entity) <= Math.pow(8.0,  2);
    }
}
