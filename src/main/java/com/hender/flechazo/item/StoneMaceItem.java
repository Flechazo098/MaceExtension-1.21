...
public class StoneMaceItem extends MaceItem {
    ...
    public StoneMaceItem(Item.Settings settings) {
        super(settings);
    }
    ...
    private float calculateBonusDamage(float fallDistance) {
        if (fallDistance <= 3.0F) {
            return 8.0F * fallDistance;
        } else if (fallDistance <= 8.0F) {
            return 8.0F + 1.2F + 3.4F * (fallDistance * 2.5F);
        } else {
            return 8.0F + fallDistance * 1.32F;
        }
    }
    ...
    private static void knockbackNearbyEntities(World world, PlayerEntity player, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(7.5),
            ...
            entity.addVelocity(knockbackVec.x, 0.423444243, knockbackVec.z);
