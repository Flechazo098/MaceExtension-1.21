...
public class DiamondMaceItem extends MaceItem {
    private static final float ATTACK_DAMAGE_MODIFIER_VALUE = 15.4F; // 自定义攻击伤害
    ...
    private static final float KNOCKBACK_POWER = 0.487F; // 自定义击退力量

    public DiamondMaceItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public void postDamageEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        stack.damage(1, attacker, EquipmentSlot.MAINHAND);//消耗的耐久
    ...
    private float calculateBonusDamage(float fallDistance) {
        if (fallDistance <= 3.0F) {
            return 15.4F * fallDistance;
        } else if (fallDistance <= 8.0F) {
            return 15.4F + 1.4F + 3.56F * (fallDistance * 4.0F);
        } else {
            return 15.4F + fallDistance * 2.0F;
        }
    }
    ...
    private static void knockbackNearbyEntities(World world, PlayerEntity player, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(9.0),
            ...
            entity.addVelocity(knockbackVec.x, 0.8999999999, knockbackVec.z); // 添加击退效果,2.8是击退的垂直距离
    ...
    private static Predicate<LivingEntity> getKnockbackPredicate(PlayerEntity player, Entity attacked) {
        return entity -> !entity.isSpectator() && entity != player && entity != attacked && !player.isTeammate(entity) &&
                attacked.squaredDistanceTo(entity) <= Math.pow(9.0,  2);
    }
