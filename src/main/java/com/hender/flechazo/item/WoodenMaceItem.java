...
public class WoodenMaceItem extends MaceItem {
    ...
    public WoodenMaceItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public boolean canRepair(ItemStack stack, ItemStack ingredient) {
        return ingredient.isOf(Items.STICK);
    }
    ...
    private float calculateBonusDamage(float fallDistance) {
        if (fallDistance <= 3.0F) {
            ...
            return 4.0F + 1.0F + 3.0F * (fallDistance * 2.0F);
        } else {
            return 4.0F + fallDistance * 1.1F;
        }
    }
    ...
    private static void knockbackNearbyEntities(World world, PlayerEntity player, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(7.0),
            ...
            entity.addVelocity(knockbackVec.x, 0.37656645, knockbackVec.z);
    ...
    private static Predicate<LivingEntity> getKnockbackPredicate(PlayerEntity player, Entity attacked) {
        return entity -> !entity.isSpectator() && entity != player && entity != attacked && !player.isTeammate(entity) &&
                attacked.squaredDistanceTo(entity) <= Math.pow(7.0,  2);
    }
}
