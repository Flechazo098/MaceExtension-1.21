...
public class NetheriteMaceItem extends MaceItem {
    ...
    public NetheriteMaceItem(Item.Settings settings) {
        super(settings);
    }
    ...
    private float calculateBonusDamage(float fallDistance) {
        if (fallDistance <= 3.0F) {
            return 20.0F * fallDistance;
        } else if (fallDistance <= 8.0F) {
            return 20.0F + 1.4F + 3.56F * (fallDistance * 4.0F);
        } else {
            return 20.0F + fallDistance * 2.56F;
        }
    }
    ...
    private static void knockbackNearbyEntities(World world, PlayerEntity player, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(10.0),
            ...
            entity.addVelocity(knockbackVec.x, 1.12133123, knockbackVec.z);
